...
export default function ShoppingPage() {
  ...
  if (items.isError) {
    return (
      <EmptyState emoji="😕" title="Couldn't load shopping" hint="Check your connection and try again." action={<Button onClick={() => items.refetch()}>Retry</Button>} />
    );
  }
  ...
  return (
    <div>
      ...
      <div className="mt-3 flex flex-col gap-2">
        {open.map((i) => (
          <Card key={i.id} className="flex items-center gap-3 py-2">
            ...
            <Button tone="quiet" disabled={purchase.isPending} onClick={() => purchase.mutate({ id: i.id })} aria-label={`Buy ${i.name}`}>
              Buy
            </Button>
          </Card>
        ))}
      ...
      {open.length === 0 && (
        <EmptyState emoji="🛒" title="List is empty" hint="Running low on something? Add it here." />
      )}
      <Card className="mt-3">
        <form onSubmit={onCreate} className="flex gap-2">
          <Field label="Add item" value={name} onChange={(e) => setName(e.target.value)} placeholder="Rice" />
      ...
      {done.length > 0 && (
        <details className="mt-4">
          <summary className="text-muted text-sm font-semibold">Bought ({done.length})</summary>
