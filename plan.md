...
## 16. Implementation Status & Hand-off (live ledger — update at every green checkpoint)**Last updated:** **Phase C hardening: sync module LIVE-bug fixed + 3 pg suites green (core 127 ✅).** `0d8638e` — D91 write-through silently no-op'd whenever core's module was entered first (db barrel re-exports core unions D69 → evaluation cycle; init-time registry literal captured undefined tables; Next/webpack only worked by entry-order luck). Tables now resolve lazily from the live `@chorify/db` namespace (`sync.pg-registry.ts`), order-proof under ESM/webpack/vite-SSR. New pg-backed suites: **occurrences** 6 (`f651024`, harness `chorify_test_<pid>` throwaway DB + real migrations + real pgUnitOfWork; skips without Postgres) · **people** 3 (`16e2baf` R2 gates, LAST_OWNER, user+sessions+oauth cascade, cross-household 404) · **sync** 6 (`0d8638e` D91 write-through, D71 uuid idempotency, domain re-auth, D61 live audiences + CN XIX-4 filtering, delete tombstones, D92 bootstrap). Harness fixes vs draft: URL-parsed test-db connection (first-match replace mangled `postgres://`) + per-seed household codes + per-pid DB names (vitest parallel workers raced DROP/CREATE). local-db 22 ✅ · all typechecks ✅ · `next build` ✅. PRIOR: §6.8 tz day boundaries DONE (5 commits). B3 D64 client jobs green; B1.2 two-device drill 21/21 green (D90–D93). NEXT: portability pg suite → CSP baseline → Amharic layout QA micro-75 → §11.5 walkthrough. **Scope law (user, D87):** offline↔online sync only — device txt backup/restore UX deferred. **B2 D62 conversion bridge DEFERRED intentionally (user, D94):** rides the D87 deferral — revisit only when device backup/restore UX reopens.
