...
|---|---|---|
| generate-occurrences | `0 */4 * * *` | expansion + backfill, all households — safety net only: user actions materialize transactionally (§4.8); every-4h covers the midnight horizon slide + downtime backfill (D99) |
| sweep-missed | `0 * * * *` | pending ∧ dueDate<today(household tz) → missed; activity; notify creator else owners — hour boundary so wakes align (D99) |
| due-today-reminders | `0 7 * * *` | ONE digest notification per person: params `{count, previewTitles}`, deep-link `/chores` |
| backup-nudge | `0 19 * * 0` (Addis) | lastExportAt null or >14d → backup notification to owner holders — 16:00 UTC, rides the 15:50 keep-alive wake (D100) |
| prune-changes | `0 4 * * *` | feed retention ≥90d — older changes prune; stale devices bootstrap — rides the digest wake window (D100) |

Contingency (pre-decided): if pg-boss timezone-cron misbehaves → daily `tick` job computes per-household local times and enqueues one-off jobs; behavior contract unchanged. All jobs force-runnable via `POST /admin/jobs/:name/run` (WORKER_ADMIN_TOKEN bearer); `GET /health`, `GET /admin/jobs` (last runs from jobs_audit). pg-boss caveat (verified v10.4.2 source): cron ticks enqueue only when the tick lands ≤60s after the schedule's previous fire time — a stopped worker skips missed ticks with no catch-up on wake. Hosting caveat (D99): on idle spin-down hosting (Render free), an external keep-alive heartbeat must warm the worker before boundary minutes or scheduled jobs silently skip.
...
Six LIGHT pastel themes (D96) — `sky` (default, reference-screenshot palette), `peach`, `caramel`, `mint`, `butter`, `rose` — all built from one "kids decorated room" recipe: long low-delta gradient wash across the page background (3 stops, ~168°, lightness drift ≤ ~4%), near-white gradient card washes, low-contrast ink, hairline `--color-line` borders, layered warm-tinted shadows (soft/lift/glow), 12/18/26px radii, and the FAB always wearing the theme primary (`--chorify-fab`/`--chorify-fab-ink`). Per-theme crayon accents (`--color-crayon-1..4`) tint chips, task-row edge stripes, pickers and section decorations; scenario task rows (action / done / upcoming) plus the check-off wash pair (`--chorify-wash-*`) ride the same tokens. Raw values live only in `styles/tokens.css` (D77); Tailwind `@theme inline` maps `var(--chorify-*)`; theme ids are mirrored in `theme/themes.ts` + the layout boot-script allowlist; legacy stored ids migrate to `sky` on load. Emoji avatars everywhere (people/rooms/routines/assets) — playful family feel, explicitly anti-corporate. Bespoke kit on Radix primitives (`apps/web/src/components/ui/*`): Button, Card, Sheet (bottom-sheet mobile / centered dialog desktop), Switch, Select, Tabs, Field, Chip, EmptyState, Skeleton, Toast, TaskRow variants.
...
### 5.4 Route map
`/login` · `/onboarding` · `/` Today (Home tab) · `/chores` (+`/chores/[id]`, Tasks tab) · `/pantry` (Pantry tab: supplies + shopping + deadlines; `/supplies` and `/shopping` redirect here) · `/family` (Family tab: per-member cards; `/household` remains the full management view) · `/routines` (+detail) · `/household` people · `/setup/roles` (+role editor) · person sheet (modal from /household) · `/setup` hub · `/setup/rooms`, `/home`, asset detail · `/activity` · `/notifications` · `/settings` · `/more` · `/print`.

### 5.5 Screen behaviors
- **Global header (D97):** left — profile chip (avatar tile, display name, role line with owner star) opening the account switcher, then the time-of-day greeting (hidden on narrow phones); right — sync chip (md+), compact EN/አማ language pill, notification bell with unread dot.
- **Navigation (D97):** five destinations — Home (`/`, today + attention), Tasks (`/chores`), Pantry (`/pantry`, running-out + shopping list + dates to remember), Family (`/family`, per-member today/done/missed with an Everyone/Mine filter for `view_people` holders; members see only their own card), Settings (`/settings`). Desktop rail adds Routines, Home inventory, Activity, Notifications. FAB floats above the bar; nav carries no color tint — active state is the filled icon variant.
...
5. **Sync engine** ✅ DONE — LWW applier + coalesced conflict notifications + FlushScheduler (`615c4d6`, 16 local-db tests green), web wiring (`d2ed302`), post-pull cache refresh (`9be85f9`). REMAINING: live two-device drill (§11.7 device half, Phase B1.2).
...
## 16. Implementation Status & Hand-off (live ledger — update at every green checkpoint)**Last updated:** **UI/UX iteration 1 shipped (user screenshot pass, `tmp/iterate-1`).** Filter-chip label bug fixed (all three chips showed the selected tab); gregorian dates render as locale labels ("Sep 14"), never raw ISO; i18next `CustomTypeOptions` augmentation now makes any missing `t()` key a compile error (10 latent dynamic-key sites flushed); built-in role names localize by `builtinKey` (`role.*` ×11 EN+AM — ProfileSwitcher/Chip/PersonSheet show "Responsible Child", not `responsible_child`); new stroke-SVG `Glyph` family (22 glyphs + chore/supply keyword mappers) replaces every emoji in system chrome; EmptyState speaks in glyph art; list-card v2 anatomy (min-touch rows, rounded-xl, overdue-red/done-wash state rails, warm person emoji avatars replacing initial dots); shopping rows wear cart tiles not false checkmarks; quiet-control contrast fixed (Buy/Almost-out `on-wash` tone, unselected pills muted→ink/70); Today's weekly-tally section REMOVED (user call — anti-scoreboard); fact tags say Woman/Man for adults; Shell clears the fixed FAB; Toast v2 kind-aware with glyphs + six hardcoded-English toasts moved to dict + missing-title guard; per-field passcode validation (aria-describedby); `Switch` primitive (notification prefs); `Accordion` primitive (Settings "Security & privacy" wraps passcode). Praised-and-kept: top header, pantry input+adder pattern. Commits `7c18f93` `43e0d21` `bcf70b8` `e939c65` `4519b12` `153b22c` `39141d3`. Core 132 + local-db 22 ✅, typecheck 0, `next build` green. PLANNED, not yet implemented (D101): step-down login — cold devices ask code → face grid → password; warm devices land on the face grid with zero typed fields; new additive `POST /auth/household-preview`; device remembers household + last usernames across logout. **Worker cron retune + Trigger.dev keep-alive shipped (D99–D100).** `apps/trigger` is now a real workspace package (`@chorify/trigger`, trigger.dev SDK/build/CLI pinned 4.5.16, `dirs: ["./src/trigger"]`) with a single `ping-worker` heartbeat task: `GET /health` at :50 before worker cron boundaries (hourly + 02:50 UTC), `WORKER_HEALTH_URL` from dashboard env, no secret key for pure-cron tasks, 90s fetch timeout. Generator cron relaxed `*/15` → `0 */4`, sweep moved to `:00` (D99) — pg-boss skips missed cron ticks with no catch-up (verified in v10.4.2 source), so the external heartbeat is what makes the daily Addis digest trustworthy on Render free hosting. Commits `5b6b9d6` `53c9ec3`. PRIOR: **Crafted card system + person facts shipped (D98).** Person wire contract now exposes sex/birthDate/age (server `personRowSchema` always carried them; only the web payload + device serializers needed widening — no pg/sqlite migration); identity facts editable in the person sheet (birthday preferred, loose age only when the date is unknown per §6.9, display age derived live from birthDate); fact tags on header chip, member cards and the account switcher. New isolated card components — `AccountSwitcherCard` (three-state switcher rows: active pill / inline password gate / pick target, no list shift), `MemberCard` (one shared family-member card for household wall + Family page), `CountStat` (single-number stat card; family facts strip, person-sheet grid, home weekly tally and pantry tiles now share one optical baseline). Commits `eff7525` `c1e0a11` `46031dc` `49b9107` `a545c19`. Typecheck 0, core 132 + local-db 22. PRIOR: **IA v2 shipped (D97).** Five-tab nav (Home/Tasks/Pantry/Family/Settings), Pantry attention hub (supplies+shopping+deadlines, old routes redirect), Family per-member cards with permission-gated filter, profile-chip header + EN/አማ pill + bell; hand-drawn two-variant icon set; full-height task cards; crayon avatar wall. PRIOR: **Visual system v2 shipped (D96).** `4eb63f1` six pastel gradient-wash themes (tokens, registry, migration, swatch picker) + `39af62d` primitive restyle (washes/glow/spring) + `99c7395` shell ambient + `082eb00` scenario task rows + `38b2da4` pastel selectors + `8fb6b09` 3-breakpoint tuning. All light, FAB=primary, legacy ids → sky. §5.2 rewritten. PRIOR: **10 themes shipped (3 kept + 7 experiments).** `292377e` infra (FAB reads `--chorify-fab`/`--chorify-fab-ink`, toggle rows wrap, Settings labels via i18n dict EN+AM) + `9c5de9c` glacier (screenshot reference: blue primary, tan FAB) + `6566322` honey + `29bb7b1` garden + `992414f` rose + `fbdc8a1` buna + `98a7065` olive + `9235196` meskel. Registration per theme = tokens block + `themes.ts` + boot-script whitelist + dict keys; tweak loop is `tokens.css`-only via `/theme-preview`. Live probe: all 10 apply with distinct primary/fab/bg. Typecheck 0, core 132 + local-db 22, `next build` green. **CSP baseline verified live.** Headers served on every route (`Content-Security-Policy` self + documented unsafe-inline/eval scope, `nosniff`, `DENY`, strict referrer) — §8/D36 mitigation confirmed, not just configured. **§11 pre-walkthrough automation green.** `24b3a5e` worker comment micro-sync; suites core 132 + local-db 22 ✅, web typecheck 0, `next build` green (20 routes), PWA assets present (manifest/icons/SW/wasm all 200), `passcodeNeedsDurable` in Dict type + EN + AM. Live §11.2–11.4 on :3001: login → /today {1 today, 1 missed, 17 upcoming} → complete → double-complete `409 ALREADY_DONE` → view-as mutation `403 VIEW_AS_READONLY` (frozen copy) → passwordless switch to Sami (Child-scoped map) → export `### CHORIFY-HOUSEHOLD v1` → same-household re-import `409 IMPORT_CONFLICT` with counts → forged v99 `409 IMPORT_TOO_NEW`. Headless route sweep 19/19 ✓. Ops note: stale Sep10 dev servers were squatting on :3000 (pre-fix code, /login 500) — killed by explicit pid; fresh server on :3001. **Device passcode gate shipped (D63 unit 1).** `b17c850` — LockScreen on app entry (sessionStorage entry key, fail-open only on gate-read errors; recovery per D63 = online sign-in resets the gate); Settings set/change/remove with the offline data-loss warning; gate row persisted in the device DB (`device_settings`, device migration 0002), never localStorage; memory-tier refusal surfaced as `settings.passcodeNeedsDurable` copy (EN+AM); offline household creation refuses the memory tier rather than faking durability. Shipped inside the same unit — **the capability ladder was structurally broken and is now fixed worker-authoritative:** the window-side `createSyncAccessHandle` probe misclassified every real browser (SAH is a worker-only API), so OPFS never engaged and the web app silently ran db-less everywhere; the worker now attempts OPFS (`installOpfsSAHPoolVfs` + `oo1.DB(…, 'opfs-sahpool')` — note `oo1.OpfsDb` belongs to the async-proxy VFS, not the SAH-Pool build) and falls back to the §4.12 memory tier (`:memory:`), with `none` as the only db-less tier; the never-shipped `sqlite3.wasm` assets are now a generated artifact (`pnpm --filter @chorify/web wasm`, gitignored). E2E: set → lock on fresh session → wrong-code rejected with hint → unlock → remove, zero page errors; core 132 + local-db 22. Multi-tab SAH exclusivity documented: first tab holds the durable handle, a second concurrent tab degrades to memory. **At-rest envelope (D63 option) assessed and DEFERRED (D95):** sqlite-wasm's OPFS VFS has no page-cipher hooks; an app-level encrypted-blob envelope is viable but deferred; SQLCipher-wasm rejected (no maintained browser build). **Person sheet + history strip + small batch shipped; 15/15 routes green (core 132 ✅, local-db 22 ✅, typecheck 0).** `3164dd6` — person sheet complete (§5.5/D16): week/month stat buckets, 8-week sparkline, most-completed breakdown (facts only), per-domain permission customizer pre-filled from the role matrix with override highlights (CN §25–26), access-boundary sentence (micro-53), Print weekly shortcut; PersonPayload gained permissionOverrides (device twins now persist it — server always did); finished counts key on completedBy so TAKEN chores count (claim=complete §6), missed stays assignment-based. Import lesson: `@chorify/core` root barrel in a client component drags argon2 native into the browser bundle (login 500) — always subpath-import (`core/permissions`). `07e33c9` — chore-detail history strip: terminal occurrences newest-first, "taken by {name}" claim annotation, 3-row collapse + show-all. `f811ebf` — §4.14 contract routes: GET /permissions (catalog+presets), GET /meta (public by design: locales/currencies/starter profile/googleEnabled+clientId); /setup hub exists (CN §99) — route sweep now 15/15. `f1a0570` — Continue-with-Google (D50): plain OAuth redirect flow (no GIS script, CSP stays 'self'), button gated on /meta.googleEnabled, callback page exchanges the code via POST /auth/google. `11066b0` — micro-67 verbatim IMPORT_CONFLICT/IMPORT_TOO_NEW toasts; calendar pref APPLIED (§5.2): Ethiopic dates via core JDN converter, month names in i18n cal.m1–m13 (EN transliteration + full አማርኛ script), "Aug 25 · Meskerem 19" in Both mode, on Today+Chores. `4306588` — micro-24: notification deep-links (cards navigate to persisted linkPath) + unread dot on the More tab (device+server). Probe-verified headlessly: override round-trip (toggle broke Hana's access → server enforced → clear fixed — authorization working as designed), sparkline/breakdown/boundary render, calendar modes, Google gating. **NEXT: §11.5 walkthrough (user-driven) → fix findings, Amharic layout QA, full §11 re-run → ledger close → v1.** PRIOR: **Chores power features shipped (core 132 ✅, typecheck/build green).** `7b41241` — composer now exposes the FULL §4.8 schedule grammar core always supported: every_n_days/every_n_weeks (interval), weekly (weekday chips), monthly (day-of-month), range (end date), rotation builder + period, subtask repeater, emoji picker, live "Next up" preview driven by the same `expandRule` engine that generates occurrences (§5.5 law). Device-twin create/read paths widened to persist every rule field — they previously hardcoded interval/anchor/rotation to null (latent device-mode data-loss bug caught while wiring). Feature RuleInput/RuleRecord now mirror core. `8f8a63c` — chore detail: natural-language rule readout (CN micro-11/32) from real rule rows incl. rotation roster, occurrence-level "This week only" reassign sheet (§6.3, gated by responsibilities.reassign; verified: 09-10 moved Daniel→Abebe while rotation kept 09-13/16), Assign-again prefill (micro-29). Claim semantics confirmed = complete (§6): any member with complete=true taps ✓ on an Up-for-grabs card; completedBy records taker — no new code needed. Headless-driving lesson: CDP keyboard events drop after JS focus() without a user gesture (compositor focus); drive React controlled inputs with the native value setter + input event, real-user typing unaffected. **PRIOR: Motion kit shipped + 2 real web-layer bugs found via headless e2e (core 132 ✅).** `b70453d` — CRITICAL: every feature hook passed `queryFn: mode === 'device' ? fn : undefined` into `useApiQuery`, whose spread ORDER clobbered the axios fetcher with undefined in server mode → all TanStack queries mounted fetcher-less → every page dead for synced households (device mode worked; regression rode the A2/A4 mode-branching, caught only when the user's §11.5 walkthrough hit it). Fix: destructure the device override, apply only when defined. `5afc1c6` — §5.3 motion kit: ChoreCheck (spring pop + per-theme green wash via `--chorify-wash-*`), SwipeCard (pointer swipe-right-to-complete, pan-y safe), theme-token confetti + vibrate(10) fired centrally in useOccurrenceAct, TickNumber, stagger-item/page-enter/sheet-panel, all reduced-motion gated. `e924a30` — second live bug: GET /occurrences returned RAW rows while the §4.14 wire type promises `title` → Chores list rendered empty titles in server mode; added `titledOccurrenceSchema` + `listRangeTitled` (RQB responsibility title join), route swapped, pg case #7 pins it (core 132). `7962350` — e2e smoke hydration wait. NEW REGRESSION GUARD: `scripts/e2e-smoke.mjs` (puppeteer-core + system Chrome): login → 15-route sweep (console/HTTP-4xx/h1 capture) → Today completion probe. LESSON: never `next build` while `next dev` serves the same .next — dev must restart after build. Headless-verified: 14/15 routes render real data (only /setup 404s — known gap), completion PATCH 200 + Undo toast + server state match.
**PRIOR: FOUR pg suites green (core 131) + 3 live bugs fixed.** `4ec3596` — portability had the same core↔db cycle poisoning (SECTION_TABLES literal → undefined tables) AND a real D62 blocker: importForAdoption inserted ISO-string timestamps into pg timestamp columns → every fresh adoption 500'd. Fixed via the shared lazy kernel `core/db-pg-tables.ts` + `coerceTimestamps` (same coercion as the sync applier). `c393be0` — D91 write-through silently no-op'd whenever core's module was entered first (db barrel re-exports core unions D69 → evaluation cycle; init-time registry literal captured undefined tables; Next/webpack only worked by entry-order luck). Tables now resolve lazily from the live `@chorify/db` namespace (`sync.pg-registry.ts`), order-proof under ESM/webpack/vite-SSR. New pg-backed suites: **occurrences** 6 (`45d2a08`, harness `chorify_test_<pid>` throwaway DB + real migrations + real pgUnitOfWork; skips without Postgres) · **people** 3 (`b71fa39` R2 gates, LAST_OWNER, user+sessions+oauth cascade, cross-household 404) · **sync** 6 (`c393be0` D91 write-through, D71 uuid idempotency, domain re-auth, D61 live audiences + CN XIX-4 filtering, delete tombstones, D92 bootstrap) · **portability** 4 (`4ec3596` export stripping/roundtrip/lastExportAt, IMPORT_CONFLICT tx-rollback zero-writes, fresh adoption incl. verbatim ids + owner-seat repoint + placeholder deletion + syncedAt flip, garbage VALIDATION_ERROR). Harness fixes vs draft: URL-parsed test-db connection (first-match replace mangled `postgres://`) + per-seed household codes + per-pid DB names (vitest parallel workers raced DROP/CREATE). local-db 22 ✅ · all typechecks ✅ · `next build` ✅. PRIOR: §6.8 tz day boundaries DONE (5 commits). B3 D64 client jobs green; B1.2 two-device drill 21/21 green (D90–D93). NEXT: CSP baseline → Amharic layout QA micro-75 → §11.5 walkthrough → full §11 re-run → ledger close → v1. **Scope law (user, D87):** offline↔online sync only — device txt backup/restore UX deferred. **B2 D62 conversion bridge DEFERRED intentionally (user, D94):** rides the D87 deferral — revisit only when device backup/restore UX reopens.

### Session 2026-09-10 — pg suites + live sync-module bug fixed
- **Live bug (`c393be0`):** D91 write-through no-op'd under ESM-first module order — the init-time `PG_ENTITY_TABLES` literal captured `undefined` tables (core↔db evaluation cycle via D69 re-exports). Probe-test diagnosis: registry keys present, values `undefined`, applier silent-returning. Fix = lazy `pgTableFor(entity)` from the live namespace (`sync.pg-registry.ts`); snapshot rewritten onto the same registry (DOMAIN_BY_ENTITY map + `dbExport()` shims for column refs). Proven by 6-case sync.pg suite; `next build` green.
- **Harness (`45d2a08`+`41b0b14`):** throwaway `chorify_test_<pid>` DB per vitest worker (shared name raced DROP/CREATE → losers skipped), real migration track, real pgUnitOfWork, skips without Postgres. Draft fixes: `new URL()` for the test-db connection (string replace mangled `postgres://` → user "chorify_test"), per-seed household codes (23505 in a persistent DB). One test-arithmetic fix: post-regen pending = 14 (below-cutoff survivor + regenerated 09-08..09-20), service was correct.
- **Suites:** occurrences 6 + people 3 + sync 6 = 15 pg-backed cases; core 127 total, all files parallel-safe.

### Session 2026-09-07 (latest) — Phase C: §6.8 tz day boundaries DONE
- **Closed the step-11 LIMITATION** ("services use system-clock UTC day boundaries"). One pure primitive, five small commits:
  - core `schedule.ts` gains `isoTodayInTz(timezone, now)` — Intl en-CA YYYY-MM-DD; +2 tests (instant straddling the Addis/UTC line both directions). core 112 ✅.
  - `OccurrencesService`: materializeRules, regenerateForward and sweepMissed look up `households.timezone` once per household-scoped call and pass `clock.now()` through the primitive (`regenerateForward` resolves the responsibility → household first).
  - `ResponsibilitiesService`: instant materialization + forward-only regeneration use the household tz.
  - worker `dueTodayReminders`: per-household `today = isoTodayInTz(household.timezone, now)` — the Addis 7am cron now digests the household's actual local day; step-11 limitation comment removed.
  - local-db `jobs.ts` drops its duplicate `isoTodayInTz` and imports core's via the client-safe `@chorify/core/schedule` subpath (D83 — one implementation, device and server agree).
- Note: `/today` route and device jobs were already tz-aware; this pass fixed the three service paths the ledger called out.

### Session 2026-09-07 (later) — B3 D64 client jobs GREEN + B2 deferred
- **B2 DEFERRED (D94, user):** the D62 conversion bridge is txt-file-related work and rides the D87 scope-law deferral — revisit only when device backup/restore UX reopens. Recorded in ledger header, §16.2, and Appendix A.
- **B3 D64 client jobs DONE** (`2ac7b4d` + `5408e40`):
  - `packages/local-db/src/jobs.ts` + `jobs.test.ts`: device twins of the worker jobs against the mirror tables — **sweep** (pending ∧ dueDate < today → missed + activity + notify creator-else-owners per §4.10, recipients resolved from mirrors) and **digest** (ONE notification per person per day, D11, `{count, previewTitles}` params, prefs-gated on `reminder` per D68). Idempotent: sweep is a status flip; digest dedupes on a per-person per-day marker row. `isoTodayInTz` computes the household-tz day via Intl (§6.8).
  - 6 new vitest cases: sweep, sweep idempotency, digest content+prefs-gating, digest dedup, reminder=false respected, runDeviceJobs single-pass. Suites: core 110 ✅ · local-db 22 ✅ · local-db + web typechecks ✅.
  - New client-safe subpath `@chorify/local-db/jobs` (D83) — `useSyncBoot` imports it directly, never the barrel.
  - `apps/web/src/lib/sync/syncClient.ts`: `useSyncBoot` now runs `runDeviceJobs(device.db, householdId)` on boot (best-effort, retried next open) BEFORE `scheduleSyncFlush()`; D67 edge cast for the async sqlite-proxy handle (same pattern as the SyncEngine ctor).
- Gotchas: drizzle sqlite enum columns (`category` in notifications) reject plain `string` at the insert typecheck — type helper params with the literal union from the schema.
...
- **A2** `bfdcce3`: device-mode Today reads via the shared pure aggregate (`@chorify/core/today`) — device mirror rows satisfy its input contracts without casts (D67: reuse pure rules, not pg-bound services).
- **A3** `9a6a190`: device-mode mutations for every feature flow — `lib/device/writes.ts` applies locally then queues `pending_ops`; `useDeviceMutation` twin mirrors `useApiMutation`'s signature so screens stay mode-agnostic.
- **A5** `d2ed302`: sync engine wired into the app — `lib/sync/syncClient.ts` singleton (engine built lazily against the memoized device DB; identity read live from the RTK store) + FlushScheduler with DOM timers (write-ack/online/focus/visibility/30s tick) + status bus → SyncStatus chip. Engine flushes ONLY when token + household code both exist; offline-only households never hit the network. 401/403 during push leaves `pending_ops` intact (ack-clears on success only).
- **A4** `b4e4ddd`: device-mode reads for every surface — `lib/device/reads.ts` (today, occurrences range, chore detail, activity feed, notifications inbox, prefs) with shapes twin to the /api/v1 routes.
- `0aa7ea7`: instant passwordless profile switching for device households (every device person is passwordless per D49; D38 cache-clear on switch).
...
- **B1.1** `9be85f9`: post-pull TanStack invalidation — a flush that applied/rejected pulled changes calls `invalidateApiCache()` (NOT the D38 wholesale clear; identity untouched); push-only flushes skip the refetch churn. Closes the A5 loose end where remote edits stayed invisible until remount.

### Session 2026-09-06 — Amharic parity + verification pass
- **Step 18a DONE** (4 commits): `a189d0f` dict contract expansion + chores screens (fixed tab-callback `t` shadowing; finished `chores/[id]`+`/new` lost to a mid-run script crash) · `125ba58` ops screens (shopping/supplies/routines/home/more) · `b9b42fe` household/roles/ProfileSwitcher/Sheet/PersonSheet + feature-layer toasts · `1bab65b` social/print/settings/onboarding. Typed `Dict` grew to 232 keys with EN/AM parity type-forced; runtime audit proves all 164 literal `t()` keys resolve, dynamic `nav.*` prefixes valid. `theme-preview` deliberately left hardcoded (dev-only mock, not a §5.4 route — user decision).
- **§11.1 ✅** core 97/15 · local-db 16/4 · all 5 workspaces typecheck · `next build` green (20 routes).
- **§11.2 ✅ live:** login hana → /today {2 today,15 upcoming} → complete Trash → activity `occurrence.completed` → double-complete `409 ALREADY_DONE` → Sami passwordless switch (me=Sami) ↔ Hana `PASSWORD_REQUIRED`/`WRONG_PASSWORD`/ok → view-as mutation `403 VIEW_AS_READONLY`.
- **§11.3 ✅ live:** admin force-runs — generate idempotent (0 new), sweep clean, digest fired (1), jobs_audit listing intact.
- **§11.4 ✅ live:** export → `My-Household-2026-09-06.txt` + v1 header; same-household re-import → `IMPORT_CONFLICT` with per-table counts; forged v99 → `IMPORT_TOO_NEW`.
- **§11.7 partial ✅ live:** uniform generic error for unknown code/username/wrong password; Postgres rate limiter `RATE_LIMITED` with `retryAfterSeconds` after 5 fails.
- **PWA ✅:** `/manifest.webmanifest` 200, icons 192/512+maskable in place, SW registered prod-only.
- **OPEN for next session:** §11.5 interactive walkthrough needs human eyes (confetti+Undo, አማርኛ toggle on every screen, offline outbox replay, print preview, view-as UI) · §11.7 two-device LWW drill · R2/LAST_OWNER live checks (already proven in earlier session smokes, spot-recheck after any auth changes).
- Infra note: session-kill reaps non-detached dev servers — use `setsid nohup pnpm dev &` for a surviving pair; Next dev needs ~15s cold start (curl races fail with 000).

### Prior hand-off (step-11 checkpoint)
Backend COMPLETE. Tree clean at HEAD `cc40173`. §12 steps 3–11 ✅ COMPLETE (full API + worker). Suites: core 97/15 ✅ · local-db 16/4 ✅ · all packages typecheck clean + reseeded pristine. NEXT: steps 12b–19 screens → Amharic parity → §11 pass. DONE step 12b-shell (`0bcd264`, 371 lines single surface): Sheet/Toast+Undo/EmptyState/Skeleton/Field/Watermark primitives, bottom tabs + left rail + FAB-create + greeting/SyncStatus shell (chromeless on /login + /onboarding). DONE step 13-partial (`2c84605` login + `d09194c` onboarding + `65b9ac3` D83): auth feature folder + /login screen; offline-first onboarding (device household + owner-clone + people rows + pending_ops queue + device session). D83: client-safe package subpaths (core/permissions, core/roles-rules, local-db/schema|queue|capability|apply-migrations) — client bundles must never import the barrel (drags pg/argon2/better-sqlite3). OPEN: browser-live onboarding proof + starter-chore templates (arrive with Chores create) deferred to §11.5 walkthrough. DONE step 14-partial (`1bce626`, 338 lines single surface): /today now embeds chore titles; chores feature folder (endpoints/types/queries/one-tap act mutation with Undo toast + ALREADY_DONE refresh); useApiMutation gains :id path-param filling; Today screen (Today cards + Missed strip + Attention + Coming up + contribution line, skeleton/error/unauthenticated states). LIVE: titled Trash + Dishes pending. NEXT: Chores list/detail/create. DONE step 14-chores (`652c0d9` core detail + `c44ebf5` screens, 372 lines single surface): responsibility detail read (subtask/rule row schemas) replacing list+find; /chores tabs Mine/Everyone/Overdue; /chores/[id] detail (steps, schedule chips, open occurrences); /chores/new composer (title/cadence/assignees; rotation+ranges next). NEXT: Household/people/roles screens. DONE step 15 (`30ccb5d` 534 lines — bundled view-as slice + household + switcher, split better next time + `6189566` roles): viewAsPersonId in RTK slice (client injects header from store; setSession/reset clear it), household feature (people/roles endpoints, queries, CRUD mutations), /household cards + PersonSheet (edit, stats, view-as entry, guarded delete), ProfileSwitcher sheet (passwordless instant, inline password gate), view-as banner + exit; /setup/roles editor (reset semantics note, name-only create). NEXT: Routines/Home/Supplies/Shopping screens. DONE step 16 (`ac06e9c` data + `48738e3` screens): routines/home/supplies/shopping feature folders; /routines (organizational note), /home (rooms grid + assets + inline creates), /supplies (one-tap state cycle), /shopping (buy flow, bought history). NEXT: Activity/Notifications/Settings/print. DONE step 17 (`97d7bb7` data + `2efe025` screens, 492 lines bundled — split better): activity/notifications feature folders; /activity story feed, /notifications inbox + prefs toggles, /settings (language/theme/calendar/household/export-import/signout), /more hub, /print week sheet with customize. Fixes: Endpoint gains put; route pages must not export extra bindings (Next type constraint). LIVE: prefs PUT green. NEXT: Amharic parity + PWA + §11 pass.
...
| D95 | Device at-rest encryption (D63 option) is DEFERRED past v1: sqlite-wasm OPFS VFS has no page-cipher hooks and SQLCipher-wasm has no maintained browser build; an app-level encrypted-blob envelope (household rows serialized into one AES-GCM blob, `:memory:` as working copy, passcode as unlock key) is the viable path but waits until offline-first deployment dominates. The shipped gate covers the synced-account threat model (server is system of record); offline-only risk narrows via OPFS origin-scoping. |
| D96 | Visual system v2: all ten earlier themes (family/ember/highland/glacier/honey/garden/rose/buna/olive/meskel) removed; six LIGHT pastel themes (sky default / peach / caramel / mint / butter / rose) built from one gradient-wash recipe — long low-delta page gradients, near-white card washes, low-contrast ink, per-theme crayon accents, layered warm shadows, FAB=primary in every theme; light-only by design (dark mode not shipped; revisit if needed); legacy stored theme ids migrate to sky; tasks render scenario-specific rows (action / done / upcoming) instead of one uniform card. |
| D97 | Information architecture v2: five-tab nav (Home / Tasks / Pantry / Family / Settings) replaces the four-tab emoji nav; Pantry merges supplies + shopping list + household deadlines into one attention hub (old routes redirect); Family renders per-member cards (today stream + facts-only done/missed counts, Everyone/Mine filter gated on `view_people`, members see only themselves); global header carries the profile chip (avatar/name/role → account switcher) and an EN/አማ language pill; nav state = filled icon variant, no tint. Age/gender on the header chip is DEFERRED — the person wire contract carries neither; revisit when the person schema grows. |
| D98 | Person identity facts + crafted card system: `PersonPayload` carries sex/birthDate/age, resolving the deferral in D97 — the server contract (`personRowSchema`) and both stores already had the columns, so only the web payload type + device serializers changed (no migration). Facts are editable in the person sheet: birthday preferred, loose age stored only when the birth date is unknown (§6.9), display age derived live from birthDate. `personFactsTag` renders "12 · girl" on the header chip, MemberCard and account switcher. UI cards are isolated components that own their geometry — `AccountSwitcherCard` (active pill / inline password gate / pick states, level list, no height shift), `MemberCard` (shared by household wall + Family), `CountStat` (strip + tall stat cards enforcing one optical baseline for all number displays). |
| D99 | Worker cron cadence + hosting wake strategy: `generate-occurrences` relaxes `*/15` → `0 */4` (backfill safety net only — user actions materialize transactionally per §4.8), `sweep-missed` moves `10 * * * *` → `0 * * * *` (hour boundary), daily/weekly jobs unchanged. pg-boss cron ticks fire only within 60s of the previous fire time (verified v10.4.2 source) — a sleeping worker silently skips, so an external keep-alive heartbeat (Trigger.dev free tier → `GET /health`) warms the worker before boundary minutes; `schedule()` is an `ON CONFLICT (name) DO UPDATE` upsert, so cadence changes apply on restart without migration. Refined with D100: every worker cron minute now sits on a keep-alive wake window — prune moved 03:00→04:00 UTC, backup-nudge 18:00→19:00 Addis. |
| D100 | Trigger.dev keep-alive heartbeat: `apps/trigger` is a self-contained workspace package (`@chorify/trigger`; `@trigger.dev/sdk` + `@trigger.dev/build` + `trigger.dev` CLI all pinned 4.5.16; config `dirs: ["./src/trigger"]`). One `schedules.task` `ping-worker` pings `GET /health` (`WORKER_HEALTH_URL`, set per environment in the Trigger.dev dashboard — no `TRIGGER_SECRET_KEY` needed since nothing triggers tasks programmatically; 90s fetch timeout absorbs a cold start, 2 retries). ONE dashboard-attached schedule `50 */4 * * *` (UTC): pings at 03:50/07:50/11:50/15:50/19:50/23:50, six ~15-min warm windows covering every worker cron minute — generate+prune (04:00) + the 07:00 Addis (04:00 UTC) digest ride the 03:50 window, backup-nudge (16:00 UTC, 19:00 Addis Sun) rides the 15:50 window; sweep-missed fires on whatever :00 ticks land inside a window and is state-based so detection may lag ~4h (within day-scale grace). ~186 runs/mo on the free tier. Boilerplate `example.ts` removed before first deploy. Dev loop = `trigger dev` (CLI browser login, laptop must stay open); prod = `trigger deploy`. |
| D101 | Step-down login + device household memory — the 3-field login form is retired from a single screen; **D52/D53 identity semantics unchanged** (login still = code+username+password server-side, uniform generic error on the actual login call, `/auth/login` contract untouched so Flutter is unaffected). Cold device: step 1 asks ONLY the household code (auto-uppercase letter boxes) → step 2 via a NEW additive `POST /auth/household-preview` (rate-limited, no secrets, uniform response shape/latency for unknown codes, POST so codes never land in URL logs) returning `{name, members:[{id,name,username,emoji,googleLinked}]}` → face grid to pick the member → password for the pre-filled username; Google-linked members go straight to the Google button. Warm device: zero typed fields — the device remembers the household (household incl. code persists in RTK) + per-device last usernames; the face grid lands first. Decisions (user): face grid shows ALL members (family shared device, WhatsApp Web model — the code is already the family shared secret); logout KEEPS the household memory (logout = "switch who's using it", cleared only by explicit "use a different household"); honest "no household with that code" error (enumeration risk low: rate limiting + 26⁶ code space). Phase 2 parked: optional 6-digit device PIN fast-unlock for returning sessions. |
