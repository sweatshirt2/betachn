...
| D99 | Worker cron cadence + hosting wake strategy: `generate-occurrences` relaxes `*/15` → `0 */4` (backfill safety net only — user actions materialize transactionally per §4.8), `sweep-missed` moves `10 * * * *` → `0 * * * *` (hour boundary), daily/weekly jobs unchanged. pg-boss cron ticks fire only within 60s of the previous fire time (verified v10.4.2 source) — a sleeping worker silently skips, so an external keep-alive heartbeat (Trigger.dev free tier → `GET /health`) warms the worker before boundary minutes; `schedule()` is an `ON CONFLICT (name) DO UPDATE` upsert, so cadence changes apply on restart without migration. |
| D100 | Trigger.dev keep-alive heartbeat: `apps/trigger` is a self-contained workspace package (`@chorify/trigger`; `@trigger.dev/sdk` + `@trigger.dev/build` + `trigger.dev` CLI all pinned 4.5.16; config `dirs: ["./src/trigger"]`). One `schedules.task` `ping-worker` pings `GET /health` (`WORKER_HEALTH_URL`, set per environment in the Trigger.dev dashboard — no `TRIGGER_SECRET_KEY` needed since nothing triggers tasks programmatically; 90s fetch timeout absorbs a cold start, 2 retries). Schedules are dashboard-attached: `50 2 * * *` (wakes for the 03:00 UTC 4h-grid generate + prune, staying warm through the 07:00 Addis digest) and `50 * * * *` (covers the hourly sweep). Boilerplate `example.ts` removed before first deploy. Dev loop = `trigger dev` (CLI browser login, laptop must stay open); prod = `trigger deploy`. |
