...
**Last updated:** **B3 COMPLETE — D64 client jobs green** (`c01c03e` local-db + `15ebdfe` web boot wiring): device mirrors run missed-sweep + due-today digest locally on app open (§6.35 — server jobs never see device rows), idempotent status-flip sweep, digest dedupes per person per day, prefs-gated on `reminder` (D68); `useSyncBoot` runs jobs then schedules the first flush. Suites: core 110 ✅ · local-db 22 ✅ · typechecks ✅. PRIOR: B1.2 two-device drill 21/21 green — LWW convergence + cross-flush loss notifications (D90), coalescing, domain re-auth + domain/audience pull filtering (D61), 401 queue survival, bootstrap freshness (D91 write-through), 90-day staleness auto-bootstrap (D93). NEXT: §11.5 walkthrough. **Scope law (user, D87):** offline↔online sync only — device txt backup/restore UX deferred. **B2 D62 conversion bridge DEFERRED intentionally (user, D94):** it is the txt-file-related work and rides the same deferral as D87 — revisit only when device backup/restore UX reopens.

### Session 2026-09-07 (latest) — B3 D64 client jobs GREEN + B2 deferred
