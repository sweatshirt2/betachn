...
**Last updated:** **Phase C hardening opened — §6.8 tz day boundaries DONE** (5 commits: core `isoTodayInTz` primitive → occurrences service → responsibilities service → worker digest → local-db dedupe): generation windows, forward-only regeneration, the missed sweep and the digest all compute "today" in `households.timezone` (was: system-clock UTC — the step-11 LIMITATION is closed). Suites: core 112 ✅ · local-db 22 ✅ · all typechecks ✅. PRIOR: B3 D64 client jobs green (`c01c03e` local-db + `15ebdfe` web boot wiring) — device mirrors run missed-sweep + due-today digest locally on app open (§6.35), idempotent sweep, digest dedupes per person per day, prefs-gated on `reminder` (D68); `useSyncBoot` runs jobs then schedules the first flush. PRIOR: B1.2 two-device drill 21/21 green — LWW convergence + cross-flush loss notifications (D90), coalescing, domain re-auth + domain/audience pull filtering (D61), 401 queue survival, bootstrap freshness (D91 write-through), 90-day staleness auto-bootstrap (D93). NEXT: remaining Phase C hardening (pg-backed service test harness, CSP baseline, Amharic layout QA micro-75) + §11.5 walkthrough. **Scope law (user, D87):** offline↔online sync only — device txt backup/restore UX deferred. **B2 D62 conversion bridge DEFERRED intentionally (user, D94):** it is the txt-file-related work and rides the same deferral as D87 — revisit only when device backup/restore UX reopens.
