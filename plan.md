...
## 16. Implementation Status & Hand-off (live ledger — update at every green checkpoint)**Last updated:** **Phase C hardening: pg-backed service test harness DONE (`f651024`)** — throwaway `chorify_test` DB + real migration track + real `pgUnitOfWork`; 6 occurrence-lifecycle cases green against live Postgres (tz materialization + idempotency, listRange predicates, ALREADY_DONE, sweep, forward-only regen, NOT_FOUND); skips cleanly without a server. Harness fixes vs draft: URL-parsed test-db connection (first-match replace mangled `postgres://`) + per-seed household codes. Suites: **core 118 ✅** · local-db 22 ✅ · all typechecks ✅. PRIOR: §6.8 tz day boundaries DONE (5 commits, closes the step-11 LIMITATION). PRIOR: B3 D64 client jobs green (`c01c03e`+`15ebdfe`); B1.2 two-device drill 21/21 green (D90–D93). NEXT: remaining Phase C hardening (sync/people/portability pg suites, CSP baseline, Amharic layout QA micro-75) + §11.5 walkthrough. **Scope law (user, D87):** offline↔online sync only — device txt backup/restore UX deferred. **B2 D62 conversion bridge DEFERRED intentionally (user, D94):** it is the txt-file-related work and rides the same deferral as D87 — revisit only when device backup/restore UX reopens.

### Session 2026-09-10 — pg-backed harness DONE (Phase C item closed)
- `f651024` harness + occurrence suite (6 cases) + `pg` devDep; plan-mode pickup after session-kill lost node_modules (pg dep added but `pnpm install` never ran). Harness bugs fixed vs draft: `ADMIN_URL.replace('/postgres',…)` hit the `postgres://` scheme → parsed as user "chorify_test" (28P01); now `new URL()` + pathname swap. `seedHousehold` had fixed code `PGTEST` → 23505 collisions across tests in a persistent test DB; now per-seed suffix. One test-side arithmetic fix: post-regeneration pending = 14 (below-cutoff 09-05 survivor + regenerated 09-08..09-20), not 12 — service was correct. Dead trailing select removed. D47 single-concern commit.
